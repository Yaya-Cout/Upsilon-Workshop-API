...
class ScriptsTest(TestCase):
    ...
    def test_unlisted_scripts(self):
        """
        Test that unlisted scripts are not visible to other users unless
        shared, but can be read.
        """
        # Log in as the user
        self.client.login(username=self.user['username'],
                          password=self.user['password'])

        # Create an unlisted script
        response = self.client.post(
            "/scripts/",
            {
                "name": "private_script",
    ...
    def test_private_and_unlisted_scripts(self):
        """
        Test that private and unlisted scripts are not visible to other users
        have the same behaviour than normal private scripts
        """
        # Log in as the user
        self.client.login(username=self.user['username'],
                          password=self.user['password'])
